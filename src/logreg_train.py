...
class LogisticRegressionTrainer:
    ...
    def log_progress(
        self, epoch: int, cost: float, callback: TensorBoardCallback, house_name: str
    ):
        if callback:
...
def apply_standardization(
    df: pd.DataFrame, mean: pd.Series, std: pd.Series
) -> pd.DataFrame:
    """
...
def save_model_weights(models: dict, feature_names: list):
    ...
    for house, model in models.items():
        ...
        with open(LOG_DIR / f"{house}_weights.txt", "w") as f:
            ...
            f.write(
                f"Final cost: {model.cost_history[-1] if model.cost_history else 'N/A'}\n"
            )
    ...
    try:
        ...
        for house in params.hogwart_houses:
            ...
            model = LogisticRegressionTrainer(
                params.learning_rate, params.epochs, params.optimization
            )
