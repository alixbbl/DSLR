...
def display_correlation_matrix(numeric_data: pd.DataFrame) -> pd.DataFrame:
    ...
    sns.heatmap(
        corr_matrix, annot=True, cmap="coolwarm", fmt=".2f", linewidths=0.5, cbar=False
    )
...
def scatter_plot_correlation_matrix(
    correlation_df: pd.DataFrame, threshold: float, data: pd.DataFrame
) -> None:
    """
    ...
    for i, course1 in enumerate(courses):
        for j, course2 in enumerate(courses):
            ...
            if corr_value >= threshold:
                ...
                plt.title(
                    f"Correlation: {course1} vs {course2}\nr = {actual_corr:.2f}",
                    fontsize=14,
                )
